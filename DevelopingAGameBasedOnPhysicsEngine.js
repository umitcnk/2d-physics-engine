...
class Rectangle {
  constructor(x1, y1, x2, y2, w) {
    ...
    this.pos = this.vertex[0]
      .add(this.dir.mult(this.length / 2))
...
let gameState = "PLAY";
let gameOverMenu;
let gameOverMenuScore;
const gameScreen = document.getElementById("gameScreen");
const ctx = gameScreen.getContext("2d");
gameOverMenu = document.getElementById("gameOver");
gameOverMenuScore = document.getElementById("score");
gameScreen.focus();
...
let playerBall = new Ball(20, 20, 15, 2);
playerBall.player = true;
playerBall.maxSpeed = 10;
playerBall.friction = 0.05;
playerBall.color = "purple";
...
putWallsAround(0, 0, gameScreen.clientWidth, gameScreen.clientHeight);

//Creating the random objects
for (let addBody = 0; addBody < 48; addBody++) {
  let x0 = randInt(100, gameScreen.clientWidth - 100);
  ...
  let r = 5 * m;
  {
    if (addBody % 4 === 0) {
      ...
      ballObj.setPosition(randInt(100, 1000), randInt(100, 700));
      ballObj.color = "#BF0426";
      ballObj.layer = 2;
      ballObj.maxSpeed = 50;
      randomBalls.push(ballObj);
      randomObjects.push(ballObj);
    }
    if (addBody % 4 === 1) {
      let boxObj = new Box(x0, y0, x1, y1, r, m);
      boxObj.setPosition(randInt(100, 1000), randInt(100, 700));
      boxObj.color = "#71FF33";
      boxObj.layer = 2;
      boxObj.maxSpeed = 50;
      randomBoxs.push(boxObj);
      randomObjects.push(boxObj);
    }
    if (addBody % 4 === 2) {
      let capsObj = new Capsule(x0, y0, x1, y1, r, m);
      capsObj.setPosition(randInt(100, 1000), randInt(100, 700));
      capsObj.color = "#F7870A";
      capsObj.layer = 2;
      capsObj.maxSpeed = 50;
      randomCaps.push(capsObj);
      randomObjects.push(capsObj);
    }
    if (addBody % 4 === 3) {
      let starObj = new Star(x0, y0, r, m);
      starObj.setPosition(randInt(100, 1000), randInt(100, 700));
      starObj.color = "#33E6FF";
      starObj.layer = 2;
      starObj.maxSpeed = 50;
      randomStars.push(starObj);
      randomObjects.push(starObj);
    }
  }
}
//randomObjects initial velocities
for (let i in randomObjects) {
  if (randomObjects[i].m !== 0) {
    randomObjects[i].vel.set(Math.random() * 6 - 2, Math.random() * 6 - 2);
  }
}

function gameLogic() {
  for (let i in randomBalls) {
    if (collide(randomBalls[i], playerBall)) {
      randomBalls[i].remove();
      randomBalls.splice(i, 1);
      score = score + 2;
    }
  }
  for (let i in randomBoxs) {
    if (collide(randomBoxs[i], playerBall)) {
      randomBoxs[i].remove();
      randomBoxs.splice(i, 1);
      score = score + 3;
    }
  }
  for (let i in randomCaps) {
    if (collide(randomCaps[i], playerBall)) {
      randomCaps[i].remove();
      randomCaps.splice(i, 1);
      score++;
    }
  }
  for (let i in randomStars) {
    if (collide(randomStars[i], playerBall)) {
      setState("GAME OVER");
    }
  }
  pScore = score;
  gameOverMenuScore.innerHTML = pScore;
}
...
function physicsLoop() {
  ...
  BODIES.forEach((b, index) => {
    for (let bodyPair = index + 1; bodyPair < BODIES.length; bodyPair++) {
      if (
        (BODIES[index].layer === BODIES[bodyPair].layer ||
          BODIES[index].layer === 0 ||
          BODIES[bodyPair].layer === 0) &&
...
function renderLoop() {
  ...
  ctx.fillStyle = "Black  ";
  ctx.fillText("Score: " + score, 440, 50);
}
